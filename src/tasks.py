...
@task
def restorewilayah(ctx):
    # Auto-muat schema 'wilayah' untuk region cakupan terkonfigurasi (env atau
    # SiteIdentity) dari BIG bila tabel masih kosong. Idempoten & tahan-gagal:
    # tidak boleh menggagalkan deploy.
    print("**************************restore wilayah***************************")
    try:
        ctx.run(
            f"python manage.py restore_wilayah --settings={_localsettings()}",
            pty=True,
        )
    except Exception:
        pass
