...
then
    ...
    invoke migrations
    invoke restorewilayah
    invoke prepare

    if [ ${FORCE_REINIT} = "true" ]  || [ ${FORCE_REINIT} = "True" ] || [ ! -e "/mnt/volumes/statics/geonode_init.lock" ]; then
        invoke fixtures
        # Seed lokal SETELAH fixtures (yang memuat kategori standar bahasa
        # Inggris dari initial_data.json). Dijalankan di blok init yang sama agar
        # hanya saat fresh install / FORCE_REINIT — tidak menimpa editan admin
        # pada restart biasa.
